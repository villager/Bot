...
const fs = require('fs'); // eslint-disable-line no-restricted-modules
...
class Path {
	...
	get noWrite() {
		return Boolean(global.Config && Config.nofswriting);
	}
	parentDir() {
		return new Path(pathModule.dirname(this.path));
	}
	resolve(...args) {
		return new Path(pathModule.resolve(this.path, ...args));
    }
    async isFile() {
	...
	stat() {
		return new Promise((resolve, reject) => {
			fs.stat(this.path, (err, data) => {
				err ? reject(err) : resolve(/** @type {fs.Stats} */ (data));
			});
		});
	}
	/**
	...
	readIfExists() {
		return new Promise((resolve, reject) => {
			fs.readFile(this.path, 'utf8', (err, data) => {
				...
				if (data.length > 0 && data.charCodeAt(0) === 0xFEFF) return resolve(data.slice(1));
				return resolve(data);
			});
		});
	}
	readIfExistsSync() {
		try {
			...
			if (data.length > 0 && data.charCodeAt(0) === 0xFEFF) return data.slice(1);
			return data;
		} catch (err) {
	 ...
	 */
	write(data, options = {}) {
		if (this.noWrite) return Promise.resolve();
		return new Promise((resolve, reject) => {
	 ...
	 */
	async safeWrite(data, options = {}, withDelay) {
		if (withDelay) await Promise.resolve(null);
		try {
	...
	async writeUpdate(dataFetcher, options = {}) {
		...
		let originalFetcher = true;
		while (pendingFetcher) {
			let updatePromise = this.safeWrite(pendingFetcher(), options, originalFetcher);
			FS.pendingUpdates.set(this.path, [updatePromise, null, options]);
			await updatePromise;
			originalFetcher = false;
			if (options.throttle) {
				await new Promise(resolve => setTimeout(resolve, options.throttle));
			}
			const nextUpdate = FS.pendingUpdates.get(this.path);
			if (!nextUpdate) return;
			[updatePromise, pendingFetcher, options] = nextUpdate;
		}
	...
	copySync(target) {
		if (this.noWrite) return;
		fs.writeFileSync(target, fs.readFileSync(this.path));
	}
	readdir() {
		return new Promise((resolve, reject) => {
	...
	/**
	 * Check if the file direction exists
	 */
	exists() {
	 	return new Promise((resolve, reject) =>{
	 		fs.exists(this.path, function(err, data) {
	 			err ? reject() : resolve();
	 		});
	 	});
	 }
