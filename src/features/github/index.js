...
let port = Config.Github ? Config.Github.port : 3420;
let secret = Config.Github ? Config.Github.secret : "";


let updates = exports.updates = {};

function report (str) {
	console.log("GitHub: " + str);
	if (Config.github) Bot('moonlight').send("/addhtmlbox " + str, Config.Github.room);
}
...
function startWebHook (secret, port) {
	...
	try {
		...
		console.log('Feature ' + exports.id + ' could not work without githubhook. Try "npm install githubhook" if you want to use this feature');
		return;
	}
