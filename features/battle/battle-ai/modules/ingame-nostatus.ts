...
export function setup(Data:any) {
	...
	function evaluateTeamDecision(battle:any, des:any) {
		let final = 0;
	...
	function getSwitchAverage(battle:any, s:any) {
		let final = 0;
		let p = battle.request.side.pokemon[s];
		let pokeA = battle.getCalcRequestPokemon(s, true);
		if (toId(battle.tier || "").indexOf("challengecup") >= 0) pokeA.happiness = 100;
	...
	function getDecisionValue(battle:any, decisions:any, desEnv:any, des:any, act:any) {
		if (des.type === "team") {
