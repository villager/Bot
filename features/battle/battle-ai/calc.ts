...
import * as TypeChartFile from './typechart';
...
export function getRolls  (n:any, p?:any) {
	if (!p) p = 0.85;
	let r = [];
	for (let m = n * p; m <= n; m++) {
		r.push(m);
...
export function getHazardsDamage (poke:any, conditions:any, gen:any, inverse:any) {
	let dmg = 0;
...
export function calculate (pokeA:any, pokeB:any, move:any, conditionsA:any, conditionsB:any, gconditions:any, gen:any) {
	if (!gen) gen = 7;
