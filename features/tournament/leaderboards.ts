...
const TOURS_DATA = './features/tournament/data/leaderboards.json';

export let ladder = {};

export function load() {
    ladder = JSON.parse(Tools.FS(TOURS_DATA).readSync().toString());
}
export function isConfigured(server: any, room: string) {
    if((!server.leaderboards || !server.leaderboards[room] )) return false;
    return true;
}
export function filterTier(tier: any, filter:any) {
	tier = toId(tier || "");
...
export function getConfig(server:any, room: string) {
    let res = {
		tierFilter: null,
		onlyOfficial: false,
		winnerPoints: 5,
...
export function parseTourTree(tree:any) {
	let auxobj = {};
	let team = tree.team;
...
export function parseTournamentResults(data:any) {
...
export function getPoints(server:any, room:string, user:string) {
...
export function getTop(server, room) {
...
export function getTable (server:any, room: string, n:number) {
	if (!isConfigured(server, room)) return null;
...
export function addUser(server:any, room: string, user:string, type:string, auxData?:any) {
...
export function writeResults (server: any, room:string, results:any) {
	if (!results) return;
...
export function onTournamentEnd(server:any, room:string, data:any) {
...
export function getResetHashCode (server:any, room: string) {
...
export function execResetHashCode (server:any, code:string) {
